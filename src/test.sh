...
# assert 8 'int main() { int x=1; return sizeof(x=2); }'  # cannot pass
# assert 1 'int main() { int x=1; sizeof(x=2); return x; }'# cannot pass
...
assert 1 "int main() { int i; int j = 0; int sum = $sum; for (i=0; i<=$n; i=i+1) {j = j + i * i;} if (sum==j) {return 1;} return 0; }"
